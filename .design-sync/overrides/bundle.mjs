// forked from design-sync lib/bundle.mjs — reactShim extended to also stub
// next/link, next/navigation, next/image so the scoped components (which
// call usePathname() and render next/image/next/link) don't crash outside
// a mounted Next.js app router. See cfg.libOverrides for the declaration.
//
...
export const reactShim = {
  ...
  setup(b) {
    ...
    // This repo's scoped components (BookingHoverMenu, BookingLink,
    // ScheduleLink, MobileNav, PhaseSlider via SceneImage) import next/link,
    // next/navigation, and next/image. Those only work inside a mounted
    // Next.js app-router tree; the preview bundle has no such tree, so
    // usePathname() etc. would throw. Stub them with plain DOM equivalents —
    // fine for a design-system bundle that ships to a non-Next consumer.
...
function Link(props){
  var href=props.href;
  var hrefStr=typeof href==="string"?href:(href&&href.pathname)||"#";
  var rest={};
  for(var k in props){if(k!=="href"&&k!=="children"&&k!=="prefetch"&&k!=="replace"&&k!=="scroll"&&k!=="legacyBehavior"&&k!=="passHref")rest[k]=props[k]}
  return R.createElement("a",Object.assign({href:hrefStr},rest),props.children);
}
module.exports=Link;module.exports.default=Link;`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-navigation-shim$/, namespace: 'shim' }, () => ({
      contents: `function usePathname(){return "/"}
function useSearchParams(){return new URLSearchParams()}
function useRouter(){return {push:function(){},replace:function(){},back:function(){},forward:function(){},refresh:function(){},prefetch:function(){}}}
module.exports={usePathname:usePathname,useSearchParams:useSearchParams,useRouter:useRouter};`,
      loader: 'js',
    }));
    b.onLoad({ filter: /^next-image-shim$/, namespace: 'shim' }, () => ({
      contents: `var R=window.React;
var OMIT=["src","alt","fill","style","priority","quality","sizes","loading","placeholder","blurDataURL","onLoadingComplete","unoptimized"];
function Image(props){
  var src=props.src;
  var srcStr=typeof src==="string"?src:(src&&src.src)||"";
  var fillStyle=props.fill?{position:"absolute",inset:0,width:"100%",height:"100%"}:{};
  var rest={};
  for(var k in props){if(OMIT.indexOf(k)===-1)rest[k]=props[k]}
  return R.createElement("img",Object.assign({src:srcStr,alt:props.alt||"",style:Object.assign({},fillStyle,props.style)},rest));
}
...
function sharedBuildOptions({ nodePaths, tsconfig }) {
  ...
  return {
    ...
    minify: false,
    // This repo's scoped components read NEXT_PUBLIC_* at module scope
    // (lib/contact.ts, lib/booking.ts) — process isn't a browser global, so
    // without these defines the IIFE throws ReferenceError on load. Empty
    // string is the correct value here: real contact/booking targets aren't
    // baked into a shared design-system bundle.
    define: {
